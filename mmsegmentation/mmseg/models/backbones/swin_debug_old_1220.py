'''
架构说明: swin_debug_old_1220.py
0. 仅在量化阶段使用单位化的k矩阵

1. 使用动态量化码表，使用掩膜进行非极大值抑制，实现标准的k-means聚类(仅迭代1次)，构造量化中心
2. 对单位化的K矩阵进行量化处理
...
训练结果: 
训练配置: batchsize8, 160k, ade20k, 1220架构使用swin-t预训练权重在ImageNet-1K上训练50epoch
mIoU: 
'''
...
class Cluster(nn.Module):
    def __init__(self, head_embed_dims: int, center_size: tuple=(16, 16), iterations: int=0, qk_scale: float=30.0):
        '''
        window_size : 下采样倍率
        ...
        # self.window_size = window_size              # tuple, (window_h, window_w)
        self.center_size = center_size
        self.iterations = iterations                # k-means聚类次数

        # 构造聚类中心
        self.subsample = nn.AdaptiveAvgPool2d(
            center_size                             # 默认池化为16*16的大小，作为聚类中心
        )
        # self.subsample = nn.AvgPool2d(              # 下采样，构造聚类中心
        #     kernel_size = self.window_size, 
        #     stride = self.window_size
        # )
        # self.subsample = nn.Conv2d(                 # 下采样，构造聚类中心
        #     in_channels = head_embed_dims, 
        #     out_channels = head_embed_dims, 
        #     kernel_size = self.window_size, 
        #     stride = self.window_size
        # )
        ...
        if self.iterations > 0:
            # 如果引入迭代的k-means聚类，则需要再对输入序列进行一次线性映射
            # self.proj = nn.Linear(head_embed_dims, 150, bias=False)
    ...
    def initRPE(size_src: tuple, size_tar: tuple, device: torch.device='cpu', gain: float=1.0):
        ...
        # exp操作用于处理distance中的0, [B, num_heads, L, S]
        distance_exp = torch.exp(distance).unsqueeze(0).unsqueeze(0)        # [1, 1, L, S]
        # 距离越远的token注意力增强越少(加性增强), 最大值为1*gain, 最小值可以接近0
        rpe = (1 / distance_exp) * gain                                     # [1, 1, L, S]
        return rpe

    # 初始化聚类中心(码表)
    def initCenter(self, x: torch.Tensor):
        '''
        输入: 
        ...
        c_init = self.subsample(x_temp).reshape(
            batch_size, num_heads, head_embed_dims, -1
        ).transpose(-2, -1)

        # 如果引入k-means聚类，还需要单位化，以便进行余弦相似性度量
        if self.iterations > 0:
    ...
    # 聚类中心初始化与聚类中心更新
    def getCenter(self, x: torch.Tensor):
        '''
        输入
        ...
        # 初始化聚类中心，[batch_size, num_heads, L', head_embed_dims]
        c = self.initCenter(x)
        relative_position_bias = self.initRPE((H, W), self.center_size, x.device)
        affinity_aux = None
        scale = None
        ...
        # 使用原始x构造聚类中心
        if self.iterations > 0:
            c_sum = torch.einsum('bhlm->bhm', delta_onehot).unsqueeze(-1)
            c_sum[c_sum == 0] = 1
            c = torch.einsum(
                'bhlm,bhld->bhmd', delta_onehot, identity.reshape(batch_size, num_heads, -1, head_embed_dims)
            ) / c_sum
    ...
    def forward(self, x: torch.Tensor):
        ...
        scale                   : tensor, [1, ], 缩放系数, 用于辅助损失标签的构造
        '''
        delta_onehot, c, relative_position_bias, affinity_aux, scale = self.getCenter(x)
        return delta_onehot, c, relative_position_bias, affinity_aux, scale

# 聚类中心构造与向量量化
class ClusterTemp(nn.Module):
    def __init__(self, head_embed_dims: int, window_size: tuple=(7, 7), iterations: int=0, qk_scale: float=30.0):
        '''
        window_size : 下采样倍率
        iterations  : k-means聚类次数
        qk_scale    : 基础缩放系数
        '''
        super(ClusterTemp, self).__init__()
        self.window_size = window_size              # tuple, (window_h, window_w)
        self.iterations = iterations                # k-means聚类次数
    ...
    def initRPE(self, shape_x, shape_c, device):
        ...
        '''
        _, L_ = shape_c
        batch_size, num_heads, H, W, _ = shape_x
        H_, W_ = H // self.window_size[0], W // self.window_size[1]
        N = self.window_size[0] * self.window_size[1]

        delta_index = torch.arange(L_, device=device).reshape(1, 1, L_, 1).repeat(batch_size, num_heads, 1, N)
    ...
    # 聚类中心初始化与聚类中心更新
    def getCenter(self, x: torch.Tensor, labels: torch.Tensor):
        '''
        输入
        x                       : tensor, [batch_size, num_heads, H, W, head_embed_dims], 待聚类的K矩阵
        输出
        ...
        # [batch_size, 1, 128, 128]/[batch_size, 1, 64, 64]/[batch_size, 1, 32, 32]
        labels = F.interpolate(labels, (H, W), mode='nearest')
        labels = torch.asarray(labels, dtype=torch.uint8) + 1       # 0-149+255 -> 0->150

        # 分块，模拟下采样的过程，更加精细
        num_classes = 151
        stride = (self.window_size[0], self.window_size[1])
        bs = stride[0] * stride[1]                          # block size
        nb = H * W // bs                                    # num blocks
        labels_unfold = F.unfold(                           # [B, nb, bs, dim]
            labels.float(), 
            kernel_size=stride, 
            padding=0, 
            stride=stride
        ).reshape(batch_size, bs, nb).transpose(-2, -1).long()       # [B, dim*bs, nb] -> [B, nb, bs]

        # 用于区分聚类中心每一行的编码
        index_add = torch.arange(0, batch_size*nb, device=labels.device).reshape(batch_size, nb, 1) * num_classes
        ...
        padding = batch_size * nb * num_classes - counts.shape[0]    # 最后一行特殊处理
        counts = torch.cat([counts, torch.zeros(padding, dtype=torch.long, device=labels.device)], dim=0)

        # 获取下采样代表类别
        c = counts.reshape(batch_size, nb, num_classes).argmax(dim=-1)

        # 初始化聚类中心，[batch_size, num_heads, L', head_embed_dims]
        _, relative_position_bias = self.initRPE(x.shape, c.shape, x.device)
        affinity_aux = None
        scale = None

        # [B, L, dim]
        labels_onehot = F.one_hot(labels.reshape(batch_size, -1).long(), num_classes).float()
        # [B, S, dim]
        c_onehot = F.one_hot(c, num_classes).float()

        # [B, L, L']
        affinity_raw = torch.einsum('bld,bmd->blm', labels_onehot, c_onehot)
        affinity = affinity_raw  * 10
        if not relative_position_bias is None:
            affinity = affinity + relative_position_bias[:, 0, :, :]
        
        # [B, L]
        affinity = affinity.argmax(dim=-1)
        # [B, num_heads, L, L']
        delta_onehot = F.one_hot(affinity, nb).unsqueeze(1).repeat(1, num_heads, 1, 1).float()
        relative_position_bias = self.getMask((H, W), delta_onehot)
        # if H <= 32:
        #     affinity_show = relative_position_bias[0, 0]
        #     with open('./affinity_show.txt', 'w', encoding='utf-8') as f:
        #         for line in affinity_show:
        #             f.writelines('{} \n'.format(line.data))
        #     exit(0)
        ...
        c_sum[c_sum == 0] = 1
        c = torch.einsum(
            'bhlm,bhld->bhmd', delta_onehot, x.reshape(batch_size, num_heads, -1, head_embed_dims)
        ) / c_sum

        # print(delta_onehot.shape)
        # print('----------------------- {} ------------------------'.format(delta_onehot.device))
    ...
    def forward(self, x: torch.Tensor, labels: torch.Tensor):
        ...
        scale                   : tensor, [1, ], 缩放系数, 用于辅助损失标签的构造
        '''
        delta_onehot, c, relative_position_bias, affinity_aux, scale = self.getCenter(x, labels)
        return delta_onehot, c, relative_position_bias, affinity_aux, scale
...
class QuantizationAttn(BaseModule):
    ...
    def __init__(
        ...
        if self.window_size[0] > 1 or self.window_size[1] > 1:
            # self.cluster = Cluster(head_embed_dims, self.window_size, iterations=iterations)
            self.cluster = Cluster(head_embed_dims, (16, 16), iterations=iterations)
            # self.cluster = ClusterTemp(head_embed_dims, self.window_size)
    ...
    def forward(self, x: torch.Tensor, labels = None):
        ...
        输出：
            x (tensor): [B, H, W, C]
        """
        # if not self.training and (self.window_size[0] > 0 or self.window_size[1] > 0):
        #     if (not x.shape[1] % self.window_size[0] == 0) or (not x.shape[2] % self.window_size[1] == 0):
        #         print('id_stage: ', self.id_stage)
        #         print('id_block: ', self.id_block)
        #         print('x.shape: ', x.shape)
        #         print('labels.shape: ', labels.shape)
        #         print('\n')
        # if torch.any(labels == 150.0):
        #     print('Find label 150!')
        #     exit(0)
        # # print('labels min/max: {:3d}, {:3d}'.format(int(labels.min()), int(labels.max())))
        ...
        if self.window_size[0] > 1 or self.window_size[1] > 1:
            ...
            delta_onehot, c, relative_position_bias, affinity, scale = self.cluster(
                k.reshape(batch_size, self.num_heads, H, W, C // self.num_heads)
            )
            # delta_onehot, c, relative_position_bias, affinity, scale = self.cluster(
            #     k.reshape(batch_size, self.num_heads, H, W, C // self.num_heads), labels
            # )
        else:
            # [batch_size, num_heads, L, L']
            delta_onehot = torch.eye(
                H*W, device=k.device
            ).reshape(
                1, 1, H*W, H*W
            ).repeat(
            ...
            c = k
            # [batch_size, num_heads, L, L']
            relative_position_bias = Cluster.getMask((H, W), delta_onehot)
            # [batch_size, num_heads, L, L']
            affinity = None
            # [1,]
            scale = None
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape, labels = None):
        ...
        if not self.gla:    # 使用分块注意力
            ...
            x, error_quantization, affinity, scale = self.w_msa(query, labels)
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape, labels = None):

        def _inner_forward(x):
            identity = x
            x = self.norm1(x)
            x, error_quantization, affinity, scale = self.attn(x, hw_shape, labels)
        ...
        if self.with_cp and x.requires_grad:
            x, error_quantization, affinity, scale = cp.checkpoint(_inner_forward, x)
        else:
            x, error_quantization, affinity, scale = _inner_forward(x)
...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape, labels = None):
        ...
        for block in self.blocks:
            x, error_quantization, affinity, scale = block(x, hw_shape, labels)
        ...
        if self.downsample:
            ...
            return x_down, down_hw_shape, x, hw_shape, error_quantization_blocks, affinity_blocks, scale_blocks
        else:
            return x, hw_shape, x, hw_shape, error_quantization_blocks, affinity_blocks, scale_blocks
...
class SwinTransformerMod(BaseModule):
    ...
    def forward(self, x, data_samples = None):
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape, error_quantization_blocks, affinity_blocks, scale_blocks = stage(x, hw_shape, labels)
