'''
架构说明: swin_debug_old_1116.py
1. 使用下采样的k、v矩阵降低计算复杂度
2. 使用平均池化进行初步下采样，使用soft k-means进行聚类(仅迭代1次)
3. 局部分支(WindowMSA)与下采样全局分支(ClusterAttn)采用交替级联结构
4. qk_scale = head_embed_dims**-0.5，在聚类过程引入了一个初始化为1的可学习缩放系数
5. 全局分支引入相对位置编码

训练结果: /home/hjf/workspace/mmsegmentation/work_dirs/swin-tiny-patch4-window7-LN_upernet_2xb1-80k_ade20k-512x512/20241118_220715/20241118_220715.log
训练配置: batchsize8, 80k, ade20k, swin-t预训练权重
mIoU: 42.54
'''
