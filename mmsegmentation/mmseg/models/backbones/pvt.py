...
class PyramidVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            patch_embed = getattr(self, f"patch_embed{i + 1}")      # 特征图下采样的器
            pos_embed = getattr(self, f"pos_embed{i + 1}")          # 位置编码
            pos_drop = getattr(self, f"pos_drop{i + 1}")
            block = getattr(self, f"block{i + 1}")
            x, (H, W) = patch_embed(x)                              # 特征嵌入

            if i == self.num_stages - 1:                            # 最后一个stage有cls_token，所以位置编码需要单独处理
