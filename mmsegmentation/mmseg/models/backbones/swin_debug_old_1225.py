'''
架构说明: swin_debug_old_1225.py
0. 使用迭代的聚类中心
1. 使用动态量化码表，使用掩膜进行非极大值抑制，实现标准的k-means聚类(仅迭代1次)，构造量化中心
2. 除了更新聚类中心索引过程中使用余弦相似度外，其余全部采用点积相似度
3. 局部分支(WindowMSA)与量化全局分支(ClusterAttn)采用交替级联结构
4. 
5. 全局分支引入相对位置编码

训练结果: 
    /home/hjf/workspace/mmsegmentation/work_dirs/swin-tiny-patch4-window7-LN_upernet_2xb8-160k_ade20k-512x512/20241227_005225/20241227_005225.log
    /home/hjf/workspace/mmsegmentation/work_dirs/swin-tiny-patch4-window7-LN_upernet_2xb8-160k_ade20k-512x512/20241228_021126/20241228_021126.log
    /home/hjf/workspace/mmsegmentation/work_dirs/swin-tiny-patch4-window7-LN_upernet_2xb8-160k_ade20k-512x512/20241229_085550/20241229_085550.log
训练配置: 
    batchsize8, 160k, ade20k, 
    shape_c(16*16) -①> stride(8*8), 
    1220架构(比本架构早一个版本)使用swin-t预训练权重在ImageNet-1K上训练50epoch -②> 1225架构(shape_c(16*16))使用swin-t预训练权重在ImageNet-1K上训练50epoch
mIoU: 42.82 -①> 43.40 -②> 43.14
'''
...
class Cluster(nn.Module):
    def __init__(self, iterations: int=1, qk_scale: float=30.0):
        '''
        window_size : 下采样倍率
    ...
    def getCenter(self, x: torch.Tensor, delta_onehot_x: torch.Tensor):
        ...
        identity = x.reshape(batch_size, num_heads, -1, head_embed_dims)
        ...
        c_sum[c_sum == 0] = 1
        c = torch.einsum('bhlm,bhld->bhmd', delta_onehot, identity) / c_sum
...
class QuantizationAttn(BaseModule):
    ...
    def __init__(
        ...
        # 实例化聚类器
        self.cluster = Cluster(iterations)
