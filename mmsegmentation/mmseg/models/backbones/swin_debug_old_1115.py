...
class Quantizer(nn.Module):
    ...
    def initCodebook(self, x: torch.Tensor):
        ...
        '''
        _, _, N, _ =  x.shape
        c_init = torch.sum(x, dim=-2)                       # [batch_size, S, embed_dims], 将一个块内的元素求和
        c_init = c_init / N                                 # [batch_size, S, embed_dims], 均值池化
        return c_init
    ...
    def updateCodebook(self, x: torch.Tensor, c: torch.Tensor):
        ...
        c: tensor, [batch_size, S, embed_dims], 每个样本一个码表
        '''
        affinity = torch.einsum('bsd,bld->bsl', c, x)           # [batch_size, S, L], CX^T

        argmax_hw = affinity.argmax(dim=-1)                     # [batch_size, S]
        argmax_n = affinity.argmax(dim=-2)                      # [batch_size, L]
        ...
        delta_c = torch.einsum('bsl,bld->bsd', attn, x)         # [batch_size, S, embed_dims]
    ...
    def getCodebook(self, x: torch.Tensor):
        '''
        输入: 
        x: tensor, [batch_size, num_windows, N, embed_dims], 分块的K矩阵
        输出:
        c: tensor, [batch_size, S, embed_dims], 每个样本、每个头一个码表
        '''
        # 初始化聚类中心
        c = self.initCodebook(x)                            # [batch_size, S, embed_dims]
        # 通过k-means聚类更新码表c
        x = x.flatten(start_dim=1, end_dim=2)               # [batch_size, L, embed_dims]
    ...
    def forward(self, x: torch.Tensor, batch_size: int=1):
        '''
        输入
        x           : tensor, [batch_size, num_windows, N, embed_dims], X矩阵
        输出
        delta_onehot: tensor, [batch_size, num_windows, N, S], 量化的X矩阵
        c           : tensor, [batch_size, S, embed_dims], 每个样本一个聚类中心
        '''
        # 量化x
...
class ClusterAttn(BaseModule):
    ...
    def __init__(
        ...
        self.proj_q = nn.Linear(embed_dims, embed_dims, bias=qkv_bias)
        if self.window_size[0] > 1 or self.window_size[1] > 1:
            self.subsample = nn.Conv2d(embed_dims, embed_dims, kernel_size=self.window_size, stride=self.window_size)
            self.norm = nn.LayerNorm(embed_dims)
        self.proj_kv = nn.Linear(embed_dims, embed_dims * 2, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop_rate)
        self.proj = nn.Linear(embed_dims, embed_dims)
        self.proj_drop = nn.Dropout(proj_drop_rate)
    ...
    def getMask(self, size: tuple, index_onehot: torch.Tensor, gain: float=1.0):
        '''
        size: 特征图尺寸, [h, w]
    ...
    def forward(self, x: torch.Tensor):
        ...
        q = self.proj_q(x).reshape(batch_size, H*W, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        if self.window_size[0] > 1 or self.window_size[1] > 1:
            x = x.permute(0, 3, 1, 2)                   # [B, C, H, W]
            # [B, L', C]
            x_subsample = self.subsample(x).reshape(batch_size, C, -1).transpose(-2, -1)
            x_subsample = self.norm(x_subsample)        # [B, L', C]
            kv = self.proj_kv(x_subsample).reshape(batch_size, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            k, v = kv[0], kv[1]                         # [batch_size, num_heads, L', head_embed_dims]
        else:
            # [B, L', C]
            x_subsample = x.reshape(batch_size, C, -1).transpose(-2, -1)
            kv = self.proj_kv(x_subsample).reshape(batch_size, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            k, v = kv[0], kv[1]                         # [batch_size, num_heads, L', head_embed_dims]
        ...
        delta_index = torch.arange(L_, device=x.device).view(1, 1, L_, 1).repeat(batch_size, self.num_heads, 1, N)
        delta_index = delta_index.reshape(
            batch_size, self.num_heads, H_, W_, self.window_size[0], self.window_size[1]
        ).permute(0, 1, 2, 4, 3, 5).reshape(
            batch_size, self.num_heads, H_ * self.window_size[0], W_ * self.window_size[1]
        ).reshape(
            batch_size, self.num_heads, H, W
        ).reshape(
            batch_size, self.num_heads, H*W
        )                                                           # [B, num_heads, L]
        delta_onehot = F.one_hot(delta_index, L_).float()           # [B, num_heads, L, L']
        relative_position_bias = self.getMask((H, W), delta_onehot) # [B, num_heads, L, L']
        relative_position_bias = relative_position_bias.to(x.device)

        # [batch_size, num_heads, L, L']
        attn = torch.einsum('bhld,bhmd->bhlm', q, k) + relative_position_bias
        # attn = torch.einsum('bhld,bhmd->bhlm', q, k)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        # [batch_size, L, head_embed_dims]
        x = torch.einsum('bhlm,bhmd->bhld', attn, v).transpose(1, 2).reshape(batch_size, H, W, C)
