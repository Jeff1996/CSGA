...
训练结果: 
    /home/hjf/workspace/mmsegmentation/work_dirs/swin-tiny-patch4-window7-LN_upernet_4xb2-80k_ade20k-512x512/20241122_181901/20241122_181901.log
    /home/hjf/workspace/mmsegmentation/work_dirs/swin-tiny-patch4-window7-LN_upernet_2xb8-160k_ade20k-512x512/20241130_092759/20241130_092759.log
训练配置: batchsize8 -> 16, 160k, ade20k, 1118架构80k迭代权重 -> 1122架构使用swin-t预训练权重在ImageNet-1K上训练50epoch
mIoU: 43.08 -> 43.35
'''
...
class Cluster(nn.Module):
    def __init__(self, head_embed_dims: int, window_size: tuple=(7, 7), iterations: int=0, qk_scale: float=30.0):
        ...
        # )
        self.proj = nn.Linear(head_embed_dims, head_embed_dims, bias=False)
    ...
    def getCenter(self, x: torch.Tensor):
        ...
        # [batch_size, num_heads, L', head_embed_dims]
        c = self.initCenter(x)
        delta_onehot, relative_position_bias = self.initRPE(x.shape, c.shape, x.device)
...
class QuantizationAttn(BaseModule):
    ...
    def __init__(
        ...
        if self.window_size[0] > 1 or self.window_size[1] > 1:
            self.cluster = Cluster(head_embed_dims, self.window_size, iterations=iterations, qk_scale=self.scale_base)
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape):
        ...
        if not self.gla:    # 使用分块注意力
            ...
            x, error_quantization, affinity, scale = self.w_msa(query)
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape):
        ...
        def _inner_forward(x):
            identity = x
            x = self.norm1(x)
            x, error_quantization, affinity, scale = self.attn(x, hw_shape)
...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape):
        ...
        for block in self.blocks:
            x, error_quantization, affinity, scale = block(x, hw_shape)
...
class SwinTransformerMod(BaseModule):
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape, error_quantization_blocks, affinity_blocks, scale_blocks = stage(x, hw_shape)
