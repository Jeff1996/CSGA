...
class EncoderDecoderVQ(BaseSegmentor):
    ...
    def extract_feat(self, inputs: Tensor, data_samples = None) -> List[Tensor]:    # backbone特征提取
        ...
        x, loss_quantization = self.backbone(inputs)
        if self.with_neck:
            x = self.neck(x)
        return x, loss_quantization
        # return x
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        x, error_quantization = self.extract_feat(inputs)  # 增加监督信号
