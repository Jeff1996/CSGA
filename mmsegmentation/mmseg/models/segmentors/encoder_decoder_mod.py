...
class EncoderDecoderMod(BaseSegmentor):
    ...
    def extract_feat(self, inputs: Tensor, data_samples = None) -> List[Tensor]:    # backbone特征提取
        ...
        # x, loss_quantization, affinity_stages, scale_stages = self.backbone(inputs)
        if self.with_neck:
            x = self.neck(x)
        # return x, loss_quantization, affinity_stages, scale_stages
        return x
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        # x, error_quantization, affinity_stages, scale_stages = self.extract_feat(inputs)  # 增加监督信号
    ...
    def encode_decode(
        ...
        x = self.extract_feat(inputs)       # 原始结构
        # x, *_ = self.extract_feat(inputs, data_samples)                             # 在这里分割特征和量化损失
    ...
    def predict(
        ...
        # ------ 如果处于验证模式，则对输入图片进行分块 ------
        '''
        --- 输入图片尺寸：torch.Size([1, 3, 512, 683]) ---                                                                                                                                                                                   
        {'img_path': '', 'seg_map_path': '', 'scale_factor': (1.0, 1.0), 'reduce_zero_label': True, 'ori_shape': (512, 683), 'img_shape': (512, 683)}                                                                                                                                                                                                
        --- 输入图片尺寸：torch.Size([1, 3, 512, 767]) ---                                                                                                                                                                                   
        {'img_path': '', 'seg_map_path': '', 'scale_factor': (2.4741935483870967, 2.473429951690821), 'reduce_zero_label': True, 'ori_shape': (207, 310), 'img_shape': (512, 767)}     
        '''
        if not self.training:
            images = []
            H, W = inputs.shape[2:]
            stride = min(H, W)
            steps_h = H // stride + (0 if H % stride == 0 else 1)
            steps_w = W // stride + (0 if W % stride == 0 else 1)
            for index_h in range(steps_h):
                for index_w in range(steps_w):
                    if index_h == steps_h - 1:
                        start_h = H - stride
                    else:
                        start_h = index_h * stride
                    if index_w == steps_w - 1:
                        start_w = W - stride
                    else:
                        start_w = index_w * stride
                    image_crop = inputs[0, :, start_h:start_h+stride, start_w:start_w+stride]
        ...
        # print(inputs.shape)
        # seg_logits = self.inference(inputs, batch_img_metas, data_samples)          # 这里增加了data_samples参数，用于临时引入标签进行推理
        seg_logits = self.inference(inputs, batch_img_metas)                            # [batch_size, num_classes, H, W]

        # ------ 对分块结果进行拼接 ------
        if not self.training:
            seg_logits_total = torch.ones((*seg_logits.shape[:2], H, W), device=seg_logits.device) * -torch.inf
            index_subimg = -1
            for index_h in range(steps_h):
                for index_w in range(steps_w):
                    index_subimg += 1
                    ...
                    if index_w == steps_w - 1:
                        ...
                        start_w = index_w * stride
                    seg_logits_total[index_subimg, :, start_h:start_h+stride, start_w:start_w+stride] = seg_logits[index_subimg]
            seg_logits = seg_logits_total.max(dim=0, keepdim=True)[0]
