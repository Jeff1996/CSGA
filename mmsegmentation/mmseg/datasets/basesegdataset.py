...
class BaseSegDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if self.data_root is not None:
            self._join_prefix()                 # 路径拼接: 
                                                # 1. self.ann_file = self.data_root + '/' + self.ann_file
                                                # 2. self.data_prefix = self.data_root + '/' self.data_prefix
        # Build pipeline.
        self.pipeline = Compose(pipeline)
        # Full initialize the dataset.
        if not lazy_init:
            self.full_init()                    # 在这里调用父类（BaseDataset）的初始化方法
