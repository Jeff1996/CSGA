...
checkpoint_file = '/home/hjf/workspace/mmpretrain/work_dirs/pretrained_in1k/08cluster/weights_from_swin-t.pth'

model = dict(
    type='ImageClassifier',
    backbone=dict(
        type='cluster_tinyMod', 
        mlp_ratios=[4, 4, 4, 4],      # 默认是[8, 8, 4, 4]
        qk_scale=15.0,
        drop_rate = 0., 
        attn_drop_rate = 0., 
        drop_path_rate = 0.2,
...
# 学习策略配置
# for batch in each gpu is 64, 16 gpu
# lr = 5e-4 * 64 * 16 / 512 = 0.001
optim_wrapper = dict(
    optimizer=dict(
        type='AdamW',
