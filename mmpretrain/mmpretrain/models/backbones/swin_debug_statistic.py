...
class QuantizationAttn(BaseModule):
    ...
    def forward(self, x: torch.Tensor, delta_onehot_x: torch.Tensor, labels = None):
        ...
        # c_k = c_k.transpose(1, 2).half()   # [batch_size, L, num_heads, head_embed_dims]
        # c_v = c_v.transpose(1, 2).half()   # [batch_size, L, num_heads, head_embed_dims]
        ...
        # 统计码表向量利用情况
        codebook_activation_layer = torch.einsum('bhls->bhs', delta_onehot)

        return x, delta_onehot_x, error_quantization, codebook_activation_layer
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape, delta_onehot_x: torch.Tensor, labels = None):
        ...
        if not self.gla:    # 使用分块注意力
            ...
            codebook_activation_layer = None
            ...
            x, delta_onehot_x, error_quantization, codebook_activation_layer = self.w_msa(query, delta_onehot_x, labels)
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape, delta_onehot_x: torch.Tensor, labels = None):
        ...
        def _inner_forward(x, hw_shape, delta_onehot_x: torch.Tensor, labels = None):
            identity = x
            x = self.norm1(x)
            x, delta_onehot_x_dst, error_quantization, codebook_activation_layer = self.attn(x, hw_shape, delta_onehot_x, labels)
            ...
            return x, delta_onehot_x_dst, error_quantization, codebook_activation_layer

        if self.with_cp and x.requires_grad:
            x, delta_onehot_x_dst, error_quantization, codebook_activation_layer = cp.checkpoint(_inner_forward, x, hw_shape, delta_onehot_x, labels)
        else:
            x, delta_onehot_x_dst, error_quantization, codebook_activation_layer = _inner_forward(x, hw_shape, delta_onehot_x, labels)
...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape, delta_onehot_x: torch.Tensor, labels = None):
        ...
        codebook_activation_layers = []

        for block in self.blocks:
            x, delta_onehot_x, error_quantization, codebook_activation_layer = block(x, hw_shape, delta_onehot_x, labels)
            error_quantization_blocks.append(error_quantization)    # 量化损失（丢弃）
            codebook_activation_layers.append(codebook_activation_layer)
        ...
        if self.downsample:
            ...
            return x_down, down_hw_shape, x, hw_shape, delta_onehot_x, error_quantization_blocks, codebook_activation_layers
        else:
            return x, hw_shape, x, hw_shape, delta_onehot_x, error_quantization_blocks, codebook_activation_layers

@MODELS.register_module()
class SwinTransformerModSt(BaseModule):
    ...
    def forward(self, x, data_samples = None):
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape, delta_onehot_x, error_quantization_blocks, codebook_activation_layers = stage(x, hw_shape, delta_onehot_x, labels)
            ...
            codebook_activation_stages.append(codebook_activation_layers)
        ...
        return tuple(outs), error_quantization, codebook_activation_stages