'''
架构说明: swin_debug_old_1130.py
1. 使用动态量化码表，使用掩膜进行非极大值抑制，实现标准的k-means聚类(仅迭代0次)，构造量化中心
2. 对单位化的K矩阵进行量化处理
...
5. 全局分支引入相对位置编码
6. 引入drophead正则化手段（disabled）

训练结果: /home/hjf/workspace/mmpretrain/work_dirs/swin-tiny_debug_4xb64_in1k/20241129_095309/20241129_095309.log
训练配置: batchsize256, 50epoch -> 50epoch, ImageNet-1K, swin-t的预训练模型 -> 迭代50次的模型
top1: 80.570 -> 80.824

训练结果: /home/hjf/workspace/mmpretrain/work_dirs/swin-tiny_debug_4xb64_in1k/20241210_123134/20241210_123134.log
训练配置: batch_size: 4 * 64 *4(梯度累计4次, 模拟1024), lr = 4e-4(考虑batch_size的最终结果), 50 epoch, ImageNet-1K, swin-t的预训练模型, 只训练下采样全局注意力模块
mIoU: 80.6420
...
class Cluster(nn.Module):
    ...
    # 使用类平均池化初始化聚类中心(码表)
    def initCenter(self, x_k: torch.Tensor):
        '''
        输入: 
    ...
    def updateCenter(self, x: torch.Tensor, c: torch.Tensor, relative_position_bias: None):
        ...
        #     affinity = affinity.sum(dim=1, keepdim=True).repeat(1, num_heads, 1, 1)
    ...
    # 聚类中心初始化与聚类中心更新
    def getCenter(self, x_k: torch.Tensor):
        '''
        输入
        ...
        if self.iterations > 0:
            ...
            # 可导方案
            # [batch_size, num_heads, L', head_embed_dims]
            c = self.initCenter(x_k)
            with torch.no_grad():
                _, relative_position_bias = self.initRPE(x_k.shape, c.shape, x_k.device)
            # relative_position_bias = None
            for _ in range(self.iterations):
                # [batch_size, num_heads, L', head_embed_dims], [batch_size, num_heads, L, L']
                c, delta_onehot = self.updateCenter(x_k, c, relative_position_bias)
            ...
            c = self.initCenter(x_k)
            with torch.no_grad():
                delta_onehot, relative_position_bias = self.initRPE(x_k.shape, c.shape, x_k.device)

        return delta_onehot, c, relative_position_bias
    ...
    def forward(self, x_k: torch.Tensor):
        ...
        x_k                     : tensor, [batch_size, num_heads, H, W, head_embed_dims], 待聚类的K矩阵
        输出
        delta_onehot            : tensor, [batch_size, num_heads, L, L']
        c                       : tensor, [batch_size, num_heads, L', head_embed_dims], 聚类的K矩阵
        relative_position_bias  : tensor, [batch_size, num_heads, L, L']
        '''
        delta_onehot, c, relative_position_bias = self.getCenter(x_k)
        return delta_onehot, c, relative_position_bias
...
class WindowMSA(BaseModule):
    ...
    def DropHead(self, x: torch.Tensor):
        '''
        x: tensor, [batch_size, num_heads, L, head_embed_dims]
        '''
        if self.head_drop_rate > 0 and self.training:
            batch_size, num_heads, *_ = x.shape
            mask = torch.bernoulli(                                     # [batch_size, num_heads, 1, 1]
                torch.tensor(self.head_drop_rate, device=x.device).reshape(1, 1).repeat(batch_size, num_heads)
            ).unsqueeze(-1).unsqueeze(-1)
            scale = num_heads / (mask.squeeze().sum(dim=-1) + 1e-6)     # [batch_size, ]
            out = x * mask * scale.reshape(batch_size, 1, 1, 1)
            return out
        else:
            return x
    ...
    def forward(self, x, batch_size, mask=None):
        ...
        x = (attn @ v)                                                  # [B*num_windows, num_heads, N, dim]
        x = self.DropHead(x)                                            # 新增的drophead正则化
...
class QuantizationAttn(BaseModule):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.window_size[0] > 1 or self.window_size[1] > 1:
            # [batch_size, num_heads, L', head_embed_dims], [batch_size, num_heads, L, L']
            delta_onehot, c, relative_position_bias = self.cluster(
...
class SwinTransformerMod(BaseModule):
    ...
    def init_weights(self):
        ...
        #         id_block = int(name_split[name_split.index('blocks') + 1])
        #         if id_block % 2 == 1:           # 只更新修改过的量化全局block
