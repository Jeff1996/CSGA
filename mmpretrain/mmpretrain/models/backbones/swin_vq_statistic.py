...
本程序主要用于统计Transformer-VQ的码本向量利用率
'''
...
class QuantizationAttn(BaseModule):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x, error_quantization, codebook_activation_layer
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape):
        ...
        if not self.gla:    # 使用分块注意力
            ...
            x, error_quantization, codebook_activation_layer = self.w_msa(query)
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape):
        ...
        def _inner_forward_attn(x, hw_shape):
            identity = x
            x = self.norm1(x)
            x, error_quantization, codebook_activation_layer = self.attn(x, hw_shape)
            x = x + identity
            return x, error_quantization, codebook_activation_layer
        ...
        if self.with_cp and x.requires_grad and not self.gla:
            x, error_quantization, codebook_activation_layer = cp.checkpoint(_inner_forward_attn, x, hw_shape)
        else:
            x, error_quantization, codebook_activation_layer = _inner_forward_attn(x, hw_shape)
        # ffn
        if self.with_cp and x.requires_grad:
...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape):
        ...
        for block in self.blocks:
            x, error_quantization, codebook_activation_layer = block(x, hw_shape)
            error_quantization_blocks.append(error_quantization)
            codebook_activation_layers.append(codebook_activation_layer)

        if self.downsample:
            x_down, down_hw_shape = self.downsample(x, hw_shape)
            return x_down, down_hw_shape, x, hw_shape, error_quantization_blocks, codebook_activation_layers
        else:
            return x, hw_shape, x, hw_shape, error_quantization_blocks, codebook_activation_layers

@MODELS.register_module()
class SwinTransformerVQSt(BaseModule):
    ...
    def forward(self, x, data_samples = None):
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape, error_quantization_blocks, codebook_activation_layers = stage(x, hw_shape)
