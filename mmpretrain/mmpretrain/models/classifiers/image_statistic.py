...
class ImageClassifierSt(BaseClassifier):
    ...
    def __init__(
        ...
        super(ImageClassifierSt, self).__init__(
    ...
    def extract_feat(self, inputs, stage='neck'):
        ...
        x, error_quantization, codebook_activation_stages = self.backbone(inputs)
        ...
        if self.with_neck:
            x = self.neck(x)
        if stage == 'neck':
            return x, error_quantization, codebook_activation_stages
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        """
        feats, error_quantization, *_ = self.extract_feat(inputs)
    ...
    def predict(self,
        ...
        Args:
            ...
            **kwargs: Other keyword arguments accepted by the ``predict``
                method of :attr:`head`.
        """
        feats, _, codebook_activation_stages = self.extract_feat(inputs)
        ...
        if self.batchsize is None:
            self.batchsize = codebook_activation_stages[-1][-1].shape[0]
        elif self.batchsize > codebook_activation_stages[-1][-1].shape[0]:
            # 信息汇总
            print('\n--------------------------------------------------------------------------')
            for threshold in self.statistic.keys():
        ...
        # print('threshold:{}, index_stage:{}, index_layer:{}, activation rate:{}'.format(threshold, index_stage, index_layer, codebook_activation_norm_head))
        # print()
        # exit(0)
