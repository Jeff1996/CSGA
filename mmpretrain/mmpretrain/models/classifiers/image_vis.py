...
增加了delta_onehot_x数据接口，用于可视化聚类分配
'''
...
class ImageClassifierVis(BaseClassifier):
    ...
    def __init__(self,
        ...
        super(ImageClassifierVis, self).__init__(
        ...
        self.path_root_out = '/mnt/ssd/hjf/ImageNet_vis'
        if os.path.isdir(self.path_root_out):
            shutil.rmtree(self.path_root_out, ignore_errors=True)
        os.makedirs(self.path_root_out, exist_ok=True)
    ...
    def extract_feat(self, inputs, stage='neck'):
        ...
        x, delta_onehot_outs = self.backbone(inputs)
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        # loss_sum = 0
        # loss_quantization = {'loss_quantization': error_quantization.sum()} # 获取骨干网络的量化损失

        # for key in loss_quantization:
        #     if 'loss' in key:
        #         loss_sum += loss_quantization[key]
        # loss_cls = self.head.loss(feats, data_samples)                      # 获取主分类头损失
        # for key in loss_cls:
        #     if 'loss' in key:
        #         loss_sum += loss_cls[key]
        # losses.update({'loss': loss_sum})
        # losses.update(add_prefix(loss_quantization, 'backbone'))
        # losses.update(add_prefix(loss_cls, 'head'))
    ...
    def predict(self,
        ...
        Args:
            ...
            **kwargs: Other keyword arguments accepted by the ``predict``
                method of :attr:`head`.
        """
        feats, delta_onehot_outs = self.extract_feat(inputs)

        batch_size = feats[-1].shape[0]
        delta_onehot_out_stages = []
        for delta_onehot_out in delta_onehot_outs:
            _, s, h, w = delta_onehot_out.shape
            delta_onehot_out = delta_onehot_out.reshape(batch_size, -1, s, h, w)
            delta_onehot_out_stages.append(delta_onehot_out.cpu())

        # 保存原始图片（中心裁剪后的）
        target_size = 224

        for i in range(data_samples.__len__()):
            path_image_src = data_samples[i].metainfo['img_path']
            imagename_full = os.path.split(path_image_src)[-1]
            imagename = os.path.splitext(imagename_full)[0]
            path_image_out = self.path_root_out + '/' + imagename_full

            image = Image.open(path_image_src)
            ...
            # 双线性插值缩放图片，最近邻插值缩放标签（保持标签值不变）
            image = image.resize((new_width, new_height), Image.BILINEAR)

            # 中心裁剪(这里用的图片是我根据文件路径自己读取的，不是框架读取的，所以需要中心裁剪以匹配框架输出结果)
            x = (new_width - target_size) // 2
            y = (new_height - target_size) // 2
            image = image.crop((x, y, x + target_size, y + target_size))

            # # 保存图片
            # image.save(path_image_out)

            # 可视化聚类分配
            colormap = torch.randint(0, 255, (s, 3), dtype=torch.uint8)
            for stage in range(delta_onehot_out_stages.__len__()):
                delta_onehot_out = delta_onehot_out_stages[stage][i]        # 只是用最后一个stage的聚类分配， [heads, S, H, W]
                heads, s, h, w = delta_onehot_out.shape

                for head in range(heads):
                    delta_onehot = delta_onehot_out[head]                   # [s, h, w]
                    delta_index = delta_onehot.argmax(dim=0)                # [h, w]
                    delta_vis = colormap[delta_index].numpy()               # [h, w, 3]
                    delta_vis = Image.fromarray(delta_vis)                  # 转化为pillow图片
                    delta_vis = delta_vis.resize((target_size, target_size), Image.NEAREST)    # [224, 224]
                    mix = Image.blend(image.convert('RGBA'), delta_vis.convert('RGBA'), 0.4)

                    path_mix_out = self.path_root_out + '/' + imagename + '_mix_stage{}_head{:02d}.png'.format(stage, head)
                    mix.save(path_mix_out)
                    break
                # break
            # break
        # exit(0)
